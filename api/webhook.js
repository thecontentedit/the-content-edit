...
function getProEmailHTML({ customerName, licenseKey, setupUrl }) {
  ...
  const greeting = firstName ? `Hola, ${firstName} 🤍` : 'Hola 🤍';
  const features = [
    ['🎨', 'Canva Connect', 'Pega tu link y el diseño aparece directo. Sin exportar nada.'],
    ['🔗', 'Links externos', 'Pinterest, URLs, lo que sea. Solo pega y aparece.'],
    ['👤', 'Profile Preview', 'Ve cómo se verá tu bio de Instagram antes de tocar nada.'],
    ['🗺️', 'Content Map', 'Ve tu feed por pilares y sabe si está balanceado, no solo bonito.'],
    ['📅', 'Plan Grid', 'Mueve posts entre fechas con drag & drop y guarda el orden.'],
    ['🌙', 'Dark mode', 'Cambia entre modo claro y oscuro según tu aesthetic.'],
    ['⊞', 'Vista 5 columnas', 'Ve tu feed tal como se ve en Instagram desde desktop.'],
    ['▶️', 'Carruseles y reels', 'Navega carruseles y previsualiza reels sin salir de Notion.'],
  ];
  const featureCells = features.map(([emoji, name, desc], i) => {
    const isEven = i % 2 === 1;
    const isLastRow = i >= features.length - 2;
    const borderLeft = isEven ? 'border-left:0.5px solid rgba(28,25,21,0.08);padding-left:12px;' : '';
    const borderBottom = isLastRow ? '' : 'border-bottom:0.5px solid rgba(28,25,21,0.08);';
    return `<td style="width:50%;vertical-align:top;padding:10px 10px 10px 0;${borderLeft}${borderBottom}">
      <p style="margin:0 0 2px;font-size:12px;font-weight:500;color:#1C1915;">${emoji} ${name}</p>
      <p style="margin:0;font-size:11px;color:#5F5E5A;line-height:1.5;">${desc}</p>
    </td>`;
  });
  const rows = [];
  for (let i = 0; i < featureCells.length; i += 2) {
    rows.push(`<tr>${featureCells[i]}${featureCells[i+1] || '<td></td>'}</tr>`);
  }
  return emailShell(`
    <div style="padding:32px 32px 24px;">
      <h1 style="margin:0 0 6px;font-size:22px;font-weight:500;color:#1C1915;">${greeting}</h1>
  ...
  return emailShell(`
    <div style="padding:32px 32px 24px;">
      ...
      <div style="background:#ECEAE5;border-radius:8px;padding:16px 20px;margin-bottom:24px;">
        <p style="margin:0;font-size:13px;color:#1C1915;line-height:1.8;">🎨 Canva Connect<br/>🔗 Links externos<br/>👤 Profile Preview<br/>🗺️ Content Map<br/>📅 Plan Grid<br/>🌙 Dark mode</p>
      </div>
